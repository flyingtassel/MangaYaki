...
// notyet:( @include        http://manhua.dmzj.com/*

(function(window, jQuery) {
	var $ = jQuery;
	var _unsafeWindow = typeof unsafeWindow === 'undefined' ? window :
		unsafeWindow;

	_unsafeWindow.MangaYaki = {};
	_unsafeWindow.MangaYaki.version = "v0.8.1.20160109";
	...
	var MyUtils = {
		getQuerystring: function(href, name) {
			...
			var results = new RegExp("[\\?&#]" + name + "=([^&#]*)").exec(parser.search);
		...
		setQuerystring: function(href, name, val) {
			if (val === undefined) {
				val = name;
				name = href;
				href = location.href;
			...
			if (parser.search !== "") {
				var raw = parser.search.split("?")[1].split("&");
			...
			for (var prop in params) {
				if (params[prop] === undefined) {
					newSearch += encodeURIComponent(prop);
				} else {
					newSearch += encodeURIComponent(prop) + "=" + encodeURIComponent(params[
						prop]);
				}
			...
			parser.search = newSearch.slice(0, -1);
		...
		*/
		cookie: function(key, value, options) {
			...
			if (arguments.length > 1 && (!/Object/.test(Object.prototype.toString.call(
				...
				return (document.cookie = [
					encodeURIComponent(key), '=', options.raw ? value :
	...
	var MyConfig = {
		...
		storageEnabled: (function() {
			try {
				localStorage.setItem("70cecf2d4e5e444459a446b07536a03a", "MangaYaki");
		...
		load: function(bDefault) {
			var conf = {
				...
				"Image-Preload-AutoReload": false,
				"Image-Preload-Timeout": 30,

				"Image-AutoTop": true,
				"Image-AutoResize": false,
				"Image-Split-Enable": false,

				"Image-FitMode": MyConfig.View_BestFit,
				...
				"UI-Theme-Style": MyConfig.Style_iPad_White,
		...
		loadFromGlobal: function() {
			...
			if (this.gmEnabled) {
				var jconf = GM_getValue("MangaYaki-IzConfig");
		...
		loadFromLocal: function() {
			...
			if (this.storageEnabled) {
				var jconf = localStorage.getItem("MangaYaki-IzConfig");
				...
				var jconf = MyUtils.cookie("MangaYaki-IzConfig");
		...
		saveToGlobal: function(conf) {
			if (this.gmEnabled) {
				GM_setValue("MangaYaki-IzConfig", JSON.stringify(conf));
				this.gmUsed = true;
			}
		},
		...
		saveToLocal: function(conf) {
			if (this.storageEnabled) {
				localStorage.setItem("MangaYaki-IzConfig", JSON.stringify(conf));
				this.storageUsed = true;
			} else if (this.cookieEnabled) {
				MyUtils.cookie("MangaYaki-IzConfig", JSON.stringify(conf), {
					expires: 365,
					path: "/",
					domain: /[\w]+\.[\w]+$/.exec(window.location.hostname)[0]
				});
				this.cookieUsed = true;
			}
		},
		...
		removeGlobal: function() {
			if (this.gmEnabled) {
				GM_deleteValue("MangaYaki-IzConfig");
			}
		...
		removeLocal: function() {
			if (this.storageEnabled) {
				localStorage.removeItem("MangaYaki-IzConfig");
			}

			if (this.cookieEnabled) {
				MyUtils.cookie("MangaYaki-IzConfig", null, {
					path: "/",
					domain: /[\w]+\.[\w]+$/.exec(window.location.hostname)[0]
				});
			}
		...
		var methods = {
				...
				return this.each(function() {
					...
					if (img.naturalWidth) {
						$img
							...
							.trigger("izSizeEvent.izImage", {
								fitMode: Option.NoFit
							});
					} else {
			...
			fitBestSize: function() {
				return this.each(function() {
					...
					if (img.naturalWidth) {
						var imageDim = img.naturalWidth / img.naturalHeight,
							w = methods._getClientWidth.call($img),
							h = methods._getClientHeight.call($img),
							clientDim = w / h;

						if (clientDim < imageDim) {
							$img.css({
								height: 'auto',
								width: w - settings.wCorrection
							});
							$img.css({
								height: 'auto',
								width: methods._getClientWidth.call($img) - settings.wCorrection
							});
						} else {
							$img.css({
								width: 'auto',
								height: h - settings.hCorrection
							});
							$img.css({
								width: 'auto',
								height: methods._getClientHeight.call($img) - settings.hCorrection
							});
						}

						$img.trigger("izSizeEvent.izImage", {
							fitMode: Option.BestSize
						});
					} else {
						settings.isDelayFit = true;
					}
					...
					if (img.naturalWidth) {
						...
						switch (parseInt(customMode)) {
							case Option.MaxWidth:
								...
								$img.trigger("izSizeEvent.izImage", {
									fitMode: Option.Custom,
									customMode: Option.MaxWidth
								});
								break;
							...
							case Option.FixedSize:
								$img
									.css({
										width: settings.fixedSize.w > 0 ? settings.fixedSize.w : 'auto',
										height: settings.fixedSize.h > 0 ? settings.fixedSize.h : 'auto'
									})
									.trigger("izSizeEvent.izImage", {
										fitMode: Option.Custom,
										customMode: Option.FixedSize
									});
								break;
							...
							case Option.Zoom:
								$img
									.css({
										width: img.naturalWidth * settings.zoom / 100,
										height: 'auto'
									})
									.trigger("izSizeEvent.izImage", {
										fitMode: Option.Custom,
										customMode: Option.Zoom
									});
								break;
			...
			setCustomMode: function(mode, data) {
				return this.each(function() {
					...
					if (data) {
						...
						if (data.fixedSize) {
							if (typeof data.fixedSize.w !== 'undefined') {
								settings.fixedSize.w = parseInt(data.fixedSize.w);
							}

							if (typeof data.fixedSize.h !== 'undefined') {
	...
	(function($) {
		...
		var methods = {
					...
					if (settings.isAutoSplit && this.naturalWidth > this.naturalHeight) {
						switch (opts.fitMode) {
							...
							case Option.Custom:
								switch (opts.customMode) {
									...
									case Option.FixedSize:
										$img.css('width', settings.fixedSize.w ? settings.fixedSize.w *
											2 : $img.css('width') * 2);
										break;
								}
								break;
						...
						if ($img.hasClass("Left")) {
							$img
								...
								.css({
									clip: "rect(0," + imgWidth / 2 + "px," + $img.height() +
										"px,0)",
									left: Math.max((methods._getClientWidth.call($img) - imgWidth /
										2) / 2, 0)
								});
						} else {
							$img
								...
								.css({
									clip: "rect(0," + imgWidth + "px," + $img.height() + "px," +
										imgWidth / 2 + "px)",
									left: (methods._getClientWidth.call($img) - imgWidth / 2 -
										imgWidth) / 2
								});
						}
						...
						$img
							...
							.css({
								clip: 'auto',
								left: Math.max((methods._getClientWidth.call($img) - $img.width()) /
									2, 0)
							});
					}
			...
			create: function(options) {
				return this.each(function() {
					var $img = $(this),
						settings = $.extend({}, $.fn.izImageEx.defaults, options || {});
		...
		var methods = {
				...
				if (buttons.length) {
					...
					for (var i = 0; i < buttons.length; ++i) {
						...
						if ($btn.length) {
							...
							$btn = $("<input type='button' class='" + btn.classname +
								"' value='" + (btn.name ? btn.name : "") + "' />");
							if (btn.click) {
			...
			show: function(content, caption, buttons, opts) {
				...
				if (typeof content !== 'string') {
					throw new TypeError(
						'MessageBox Plugin ( Method show ): content must be a string');
				}
				...
				if (typeof opts !== 'object') {
					throw new TypeError(
						'MessageBox Plugin ( Method show ): opts must be an object');
				}

				if (!$.isArray(buttons)) {
					throw new TypeError(
						'MessageBox Plugin ( Method show ): buttons must be an array');
				}

				if (caption !== null && typeof caption !== 'string') {
					throw new TypeError(
						'MessageBox Plugin ( Method show ): caption must be a string');
				}
				...
				return this.each(function() {
					...
					$mb.find(".izMessageBox-Titlebar-CloseButton").css('display', (opts.closeButton ?
						'' : 'none'));
			...
			create: function(options) {
				...
				return this.each(function() {
					...
					var $titlebar = $("<div class='izMessageBox-Titlebar'></div>").prependTo(
						$mb);
					...
					$(
						...
						.bind("click.izMessageBox", function() {
							methods.hide.call($mb);
						})
						.appendTo($titlebar);
				});
			}
		...
		var methods = {
					...
					switch (position) {
						...
						case Option.Next:
							var columnCount = Math.floor($container.width() / settings.itemWidth),
								limit = -(settings.itemHeight * Math.ceil(settings.itemCount /
									columnCount) - $container.height()),
								currentPos = parseInt($strip.css("margin-Top")),
								interval = index ? index : settings.scrollInterval,
								newPos;
						...
						case Option.Center:
							...
							TopMargin = $container.height() / 2 - settings.itemHeight * rowNo +
								settings.itemHeight / 2;
							break;
			...
			create: function(options) {
				...
				return this.each(function() {
					...
					var $container = $("<div class='izPhotoGird-Container'></div>").appendTo(
							$pg),
					...
					$strip.css({
						top: 0,
						'-moz-transition': settings.scrollSpeed ? "margin 0.2s linear " +
							settings.scrollSpeed / 1000 + "s" : 'none',
						'-webkit-transition': settings.scrollSpeed ?
							"margin 0.2s linear " + settings.scrollSpeed / 1000 + "s" : 'none'
					});

					$container.css({
						width: settings.visibleColumnCount ? settings.itemWidth *
							settings.visibleColumnCount : settings.width
					});
					if (settings.visibleRowCount) {
						$container.css({
							height: settings.itemHeight * settings.visibleRowCount
						});
							...
							.bind('mousemove', function(event) {
								...
								if (!timeID) {
									timeID = setInterval(function() {
										...
										var isOnBeforeOneThird = cursorOffsetY < $container.height() /
											4;
										var isOnAfterOneThird = $container.height() * 3 / 4 <
											cursorOffsetY;
	...
	var OptionsDialog = (function() {
		...
		var Singleton = function() {
			...
			style.type = "text/css";
			style.textContent =
				'#mmOptionsDialog {position:fixed;z-index:1030;top:10px;left:200px;min-width:450px;border:1px solid rgba(200,200,200,0.5);box-shadow:2px 2px 3px 0px rgba(0,0,0,0.5);background:#fff;text-align:left;}#mmOptionsDialog .xxTitlebar {padding:5px 10px;border-bottom:1px solid rgba(200,200,200,0.5);background: #eee;text-align:center}#mmOptionsDialog .xxTab {margin:0 10px;}#mmOptionsDialog fieldset {margin:5px 0;padding:3px 10px;}#mmOptionsDialog input[type="checkbox"] {vertical-align:middle;}#mmOptionsDialog input[type="text"] {border:1px solid #aaa;text-align:right;}#mmOptionsDialog input[type="button"] {padding:2px 5px;border:1px solid #aaa;}#mmDialogBtn {margin:10px 0;text-align:center;}';
			document.head.appendChild(style);

			this.dialog = document.createElement("div");
			this.dialog.id = "mmOptionsDialog";
			this.dialog.innerHTML =
				'<div class="xxTitlebar">偏好配置</div><div class="xxTab"><fieldset><legend>图片选项</legend><label>向前预载张数<input type="text" id="txtImagePreloadCountPerv" size="3" /></label>&nbsp<label>向后预载张数<input type="text" id="txtImagePreloadCountNext" size="3" /></label><br /><label><input type="checkbox" id="chkImagePreloadAutoClean" />自动清除预载</label>&nbsp<label><input type="checkbox" id="chkImagePreloadAutoReload" />自动重载</label> <label>超时<input type="text" id="txtImagePreloadTimeout" size="3"  maxlength="3" />秒</label><br /><label><input type="checkbox" id="chkImageAutoTop" />自动上移边缘</label>&nbsp<label><input type="checkbox" id="chkImageAutoResize" />自动调整尺寸</label><br /><label><input type="checkbox" id="chkImageSplit" />启用分页</label></fieldset><fieldset><legend>缩放模式</legend><label for="selImageFitMode">预设模式：</label><select id="selImageFitMode"><option value="1">实际大小</option><option value="2">合适大小</option><option value="3">适应宽度</option><option value="4">适应高度</option><option value="5">自定大小</option></select><br /><div><label  for="selImageCustomMode">自定大小：</label><select id="selImageCustomMode"><option value="7">最大宽度</option><option value="8">最大高度</option><option value="9">固定大小</option><option value="10">缩放大小</option></select><br /><label>最大宽度 <input type="text" id="txtCustomMaxWidth" size="5" /></label><br /><label>最大高度 <input type="text" id="txtCustomMaxHeight" size="5" /></label><br /><label>固定大小</label> <label>宽: <input type="text" id="txtCustomFixedSizeWidth" size="5"/></label> <label>高: <input type="text" id="txtCustomFixedSizeHeight" size="5" /></label><br /><label>缩放大小 <input type="text" id="txtCustomZoom" size="5" />%</label></div></fieldset><fieldset><legend>开关灯模式</legend><label><input type="checkbox" id="chkViewOverlay" />启用开关灯模式</label><br /><label>关灯背景色 <input type="text" id="txtViewOverlayBgColor" size="7" /></label></fieldset><fieldset><legend>界面主题</legend><select id="selThemeStyle"><option value="0">原始</option><option value="1">iPad White</option><option value="2">iPad Black</option></select><br /><label><input type="checkbox" id="chkchkInfoPanelShow" />显示菜单栏</label></fieldset></div><div id="mmDialogBtn"><input type="button" class="xxOK" value="确定" />&nbsp<input type="button" class="xxCancel" value="取消" />&nbsp<input type="button" class="xxApply" value="应用" />&nbsp<input type="button" class="xxDefault" value="预设" />&nbsp<input type="button" class="xxClear" value="移除" /></div>';
			document.body.appendChild(this.dialog);

			var $styleTheme = $("#izStyle-MangaYaki-Theme");

			switch (parseInt(IzConfig["UI-Theme-Style"])) {
				case MyConfig.Style_iPad_White:
				case MyConfig.Style_iPad_Black:
					$styleTheme.text($styleTheme.text() +
						'#mmOptionsDialog {border:none;border-radius:15px;font:18px "Microsoft YaHei",sans-serif;}#mmOptionsDialog * {font:18px "Microsoft YaHei",sans-serif;}#mmOptionsDialog .xxTitlebar {border-bottom:1px solid rgba(180,180,180,0.5);border-radius:15px 15px 0 0;}#mmOptionsDialog fieldset {border:1px solid rgba(100,100,100,0.58);border-radius:8px;}#mmOptionsDialog input[type="button"] {height:44px;width:60px;margin:1px 2px;padding:2px 5px;box-shadow:0px 0px 1px 0px rgba(0,0,0,0.5);border:1px solid rgba(100,100,100,0.5);border-radius:8px;background-image: -moz-linear-gradient(top, #fff, #f8f8f8, #eee );background-image: -webkit-linear-gradient(top, #fff, #f8f8f8, #eee );color:#333;text-shadow: rgba(50,50,50,0.2) 0px -1px 0px;}#mmOptionsDialog input[type="text"] {margin:1px 3px;padding:0 5px;border-width:1px;border-style:solid;border-color:rgb(146, 146, 147) rgb(174, 174, 174) rgb(189, 189, 189);border-radius:3px;box-shadow:inset 0px 2px 3px -1px rgba(0, 0, 0, 0.2), 1px 1px 3px 0px rgba(255, 255, 255, 0.3);}#mmOptionsDialog select {margin:1px 3px;padding:0 0 0 2px;border:1px solid rgba(180,180,180,0.8);border-radius:3px;color:#000;text-shadow:rgba(50,50,50,0.2) 0px -1px 0px;}'
					);
					break;
			...
			this.txtImagePreloadCountPerv = document.getElementById(
				"txtImagePreloadCountPerv");
			this.txtImagePreloadCountNext = document.getElementById(
				"txtImagePreloadCountNext");
			this.chkImagePreloadAutoClean = document.getElementById(
				"chkImagePreloadAutoClean");
			this.chkImagePreloadAutoReload = document.getElementById(
				"chkImagePreloadAutoReload");
			this.txtImagePreloadTimeout = document.getElementById(
				"txtImagePreloadTimeout");
			...
			this.txtCustomFixedSizeWidth = document.getElementById(
				"txtCustomFixedSizeWidth");
			this.txtCustomFixedSizeHeight = document.getElementById(
				"txtCustomFixedSizeHeight");
			this.txtCustomZoom = document.getElementById("txtCustomZoom");

			this.chkViewOverlay = document.getElementById("chkViewOverlay");
			this.txtViewOverlayBgColor = document.getElementById(
				"txtViewOverlayBgColor");
		...
		Singleton.prototype = {
			...
			chkImagePreloadAutoReload: null,
			txtImagePreloadTimeout: null,
			...
			loadConf: function(bDefault) {
				...
				this.chkImagePreloadAutoReload.checked = conf[
					"Image-Preload-AutoReload"];
				this.txtImagePreloadTimeout.value = conf["Image-Preload-Timeout"];
				...
				this.selThemeStyle.selectedIndex = conf["UI-Theme-Style"];
			...
			saveConf: function() {
				this.config["Image-Preload-Count-Perv"] = parseInt(this.txtImagePreloadCountPerv
					.value, 10);
				this.config["Image-Preload-Count-Next"] = parseInt(this.txtImagePreloadCountNext
					.value, 10);
				this.config["Image-Preload-AutoClean"] = this.chkImagePreloadAutoClean
					.checked;
				this.config["Image-Preload-AutoReload"] = this.chkImagePreloadAutoReload
					.checked
				this.config["Image-Preload-Timeout"] = parseInt(this.txtImagePreloadTimeout
					.value, 10);
				...
				this.config["Image-FitMode"] = parseInt(this.selImageFitMode.selectedIndex,
					10) + 1;
				this.config["Image-CustomMode"] = parseInt(this.selImageCustomMode.selectedIndex,
					10) + 7;

				this.config["Image-MaxWidth"] = parseInt(this.txtCustomMaxWidth.value,
					10);
				this.config["Image-MaxHeight"] = parseInt(this.txtCustomMaxHeight.value,
					10);
				this.config["Image-FixedSize"].w = parseInt(this.txtCustomFixedSizeWidth
					.value, 10);
				this.config["Image-FixedSize"].h = parseInt(this.txtCustomFixedSizeHeight
					.value, 10);
				...
				this.config["UI-Theme-Style"] = parseInt(this.selThemeStyle.selectedIndex,
					10);
		...
		Singleton.evtHandler = function(self) {
			...
			var updateUI = function() {
				...
				self.app.$IzImage.izImageEx("setBgColor", IzConfig[
					"View-Overlay-BgColor"]);
	...
	var MyApp = function(settings) {
		$(
			'<div id="mmOutputList"><div id="mmOutputList-selection">S</div><div id="mmOutputList-close">╳</div><div id="mmOutputList-clientArea"></div></div><div id="mmSidePanel" class="xxPanel"></div><div id="mmInfoPanel" class="xxPanel"><div id="mmComicName"></div><div id="mmVolumeNavigation"></div></div><div id="mmControlPanel" class="xxPanel"><div id="mmControlPanel-Message"><span class="mmImageStatus"></span><span class="mmPageNum"></span></div><div id="mmTools"><a href="#" class="xxButton menu-Advanced" title="更多功能"><span>更多...</span></a><div id="mmMenuAdvanced" class="xxMenu"><div><a href="#" class="xxButton tool-Settings-Global" title=""><span>设定通用选项</span></a></div><div><a href="#" class="xxButton tool-Settings-Local" title=""><span>设定个别网站</span></a></div><div><a href="#" class="xxButton tool-List" title=""><span>输出图片地址</span></a></div><div><a href="#" class="xxButton tool-Cache" title=""><span>小图列表</span></a></div><div><a href="#" class="xxButton view-TurboMode" title=""><span>开关灯</span></a></div><div><a href="#" class="xxButton view-AutoSplit" title=""><span>切换自动分页</span></a></div></div></div><div id="mmPageNavigation"><a href="#" class="xxButton page-First" title="第一页"><span> |< </span></a><a href="#" class="xxButton page-Prev" title="上一页"><span> < </span></a><select class="page-Number"></select><a href="#" class="xxButton page-Next" title="下一页"><span> > </span></a><a href="#" class="xxButton page-Last" title="最后一页"><span> >| </span></a></div><div id="mmPageDisplay"><div id="mmFitRadio" class="xxRadioGroup"><a href="#" class="xxRadio fit-IntrinsicSize" title="实际大小"><span>I</span></a><a href="#" class="xxRadio fit-BestSize" title="适合大小"><span>B</span></a><a href="#" class="xxRadio fit-Width" title="符合宽度"><span>W</span></a><a href="#" class="xxRadio fit-Height" title="符合高度"><span>H</span></a><a href="#" class="xxRadio fit-Custom" title="自订大小"><span>C</span></a><div id="mmMenuCustomMode" class="xxMenu"><div id="mmCustomRadio" class="xxRadioGroup"><div><a href="#" class="xxRadio custom-MaxWidth" title=""><span>最大宽度</span></a> <span><input class="txtCustom-MaxWidth" size="5" /></span></div><div><a href="#" class="xxRadio custom-MaxHeight" title=""><span>最大高度</span></a> <span><input class="txtCustom-MaxHeight" size="5" /></span></div><div><a href="#" class="xxRadio custom-FixedSize" title=""><span>固定大小</span></a> <span>宽: <input class="txtCustom-FixedSize-Width" size="5"/></span> <span>高: <input class="txtCustom-FixedSize-Height" size="5" /></span></div><div><a href="#" class="xxRadio custom-Zoom" title=""><span>縮放大小</span></a> <span><input class="txtCustom-Zoom" size="5" /> %</span></div></div></div></div></div></div>'
		).appendTo(document.body);
	...
	MyApp.prototype = {
		...
		checkPageIndexOutOfBound: function(pageIndex) {
			if (pageIndex < 1) {
				this.$IzMessageBox.izMessageBox("show", "这是第一页", {
					autoClose: true
				});
				return true;
			} else if (pageIndex > this.itemsDataSource.getRowCount()) {
				this.$IzMessageBox.izMessageBox("show", "这是最后一页", {
					autoClose: true
				});
				return true;
			}
		...
		useOverlay: function(isUse) {
			if (isUse) {
				if (!this.$IzImage.hasClass("izImageEx")) {
					this.$IzImage.izImageEx({
						overlayBgColor: IzConfig["View-Overlay-BgColor"]
					});
				}
		...
		reloadImage: function($img) {
			var newSrc = MyUtils.setQuerystring($img.attr("src"), "_t", +new Date);

			$img.attr("src", newSrc);

			if (this.currentPageIndex == parseInt($img.parent().attr("title"))) {
				this.$IzImage.izImage("load", newSrc);
			}

			if (IzConfig["Image-Preload-AutoReload"]) {
				this.reloadTimer($img);
			}

			console.log($img.attr("src"));
		},

		reloadTimer: function($img) {
			// Before calling "reloadTimer", it is necessary to set the image src.

			// Image-Preload-AutoClean ==> clearTimeout
			clearTimeout($img.data("timerid"));
			...
			(function() {
				// exit if it has been removed or completed loading
				if (img.parentNode && !img.complete) {
					if (+new Date - starttime > 1000 * IzConfig["Image-Preload-Timeout"]) {
						myApp.reloadImage($img);
					} else {
						timerID = window.setTimeout(arguments.callee, 3000);
						$img.data("timerid", timerID);
						return;
					}
				}
			})();
		...
		initPhotoGird: function() {
			...
			myApp.$IzPhotoGird = $('<div id="mmPhotoGird"></div>').appendTo($(
				"#mmSidePanel")).izPhotoGird({
				...
				itemClick: function(event, data) {
					...
					if (IzConfig["Image-Preload-Count-Perv"] && myApp.currentPageIndex !==
						1) {
						var begin, count;
						if (myApp.currentPageIndex > IzConfig["Image-Preload-Count-Perv"]) {
							begin = myApp.currentPageIndex - IzConfig[
								"Image-Preload-Count-Perv"];
					...
					myApp.itemsDataSource.selectData(myApp.currentPageIndex, 1 +
						IzConfig["Image-Preload-Count-Next"]);

					if (IzConfig["Image-Preload-AutoClean"]) {
						var $img, autoCleanSize = DataSource.repairIndexToZero(myApp.currentPageIndex -
							IzConfig["Image-Preload-Count-Perv"]);
						for (var i = 0; i < autoCleanSize; ++i) {
							$img = myApp.$PhotoGirdItems.eq(i).find("img");
							clearTimeout($img.data("timerid"));
							$img.remove();
						}
					}
					...
					myApp.$ControlPanelMessage.find(".mmPageNum").html(myApp.currentPageIndex +
						msg + " / " + myApp.itemsDataSource.getRowCount());
					myApp.$PageNumberSelect.find("option:nth-child(" + myApp.currentPageIndex +
						")").prop('selected', true);
					...
					myApp.$IzPhotoGird.izPhotoGird("slideTo", MyConfig.Slide_First,
			...
			myApp.$IzPhotoGird.find(".izPhotoGird-Container").css({
				"height": $(window).height()
			});
		...
		initImage: function() {
			...
			myApp.$IzImage = myApp.$ImageBox.izImage({
				...
				click: function(event) {
					var $img = $(this),
						newPageIndex, newCommand;

					if ($img.hasClass("Splitted")) {
						if ($img.hasClass("Right")) {
							var isClickLeftOneThird = event.clientX - $img.offset().left <
								$img.width() / 2 + $img.width() / 6;
						...
						var isClickLeftOneThird = event.clientX - $img.offset().left < $img
							.width() / 3;
						if (isClickLeftOneThird) {
							newPageIndex = myApp.currentPageIndex - 1;
							if (myApp.checkPageIndexOutOfBound(newPageIndex)) {
		...
		initSidePanel: function() {
			...
			myApp.$SidePanel
				.hover(function(event) {
					var $sp = $(this),
						isHoverHandle = event.clientX - this.offsetLeft > $sp.outerWidth() -
						8;
					if (isHoverHandle) {
				...
				.bind('click', function(event) {
					var $sp = $(this),
						isClickHandle = event.clientX - this.offsetLeft > $sp.outerWidth() -
						8;
					if (isClickHandle) {
						if (parseInt($sp.css("left")) < 0) {
							$sp.css("left", 0);
				...
				.hide()
				.css({
					left: -myApp.$SidePanel.outerWidth() + 1
				});
		},
		...
		initControlPanel: function() {
			...
			$("<span>").text(myApp.itemsDataSource.getComicName()).appendTo($(
				"#mmComicName").width(150));

			$(
					'<a href="#" id="mmReloadImage" class="xxButton img-Reload" title="重新载入" accesskey="x"><span>Reload</span></a>'
				)
				.bind("click", function() {
					myApp.reloadImage(myApp.$PhotoGirdItems.eq(DataSource.repairIndexToZero(
						myApp.currentPageIndex)).find("img"));
				})
				...
				$(
					...
					.bind("click", function() {
						...
						if (prevURL) {
							...
							myApp.$IzMessageBox.izMessageBox("show", "找不到上一卷", {
								autoClose: true
							});
						}
					})
				...
				$(
					...
					.bind("click", function() {
						...
						if (nextURL) {
							...
							myApp.$IzMessageBox.izMessageBox("show", "找不到下一卷", {
								autoClose: true
							});
						}
					})
			...
			//-- OutputList
			$("#mmOutputList-selection").bind('click', function() {
				var sel = window.getSelection(),
					range = document.createRange(),
					area = $('#mmOutputList-clientArea')[0];
			...
			$("#mmMenuAdvanced").append(
				"<div style='margin-right:5px;font:12px Helvetica,Arial,sans-serif;text-align:right;'><a href='http://userscripts.org/scripts/show/135690' target='_blank' title='MangaYaki'>v" +
				_unsafeWindow.MangaYaki.version + "</a></div>");
			...
			if (MyConfig.gmEnabled) {
				$(".xxButton.tool-Settings-Global").bind('click', function(event) {
					...
					if (dlg.dialog.style.display === "none") {
						dlg
							.setTitleText("偏好设定 (通用) - Greasemonkey")
							.init({
								app: myApp,
			...
			if (MyConfig.cookieEnabled || MyConfig.storageEnabled) {
				$(".xxButton.tool-Settings-Local").bind('click', function(event) {
					...
					if (dlg.dialog.style.display === "none") {
						dlg
							.setTitleText("偏好设定 (个别) - " + (MyConfig.storageEnabled ?
								"LocalStorage" : "Cookie"))
							.init({
								app: myApp,
			...
			$(".xxButton.page-First").bind('click', function() {
				if (myApp.currentPageIndex === 1) {
					if (!myApp.$IzImage.hasClass("Splitted") || (myApp.$IzImage.hasClass(
							"Splitted") && myApp.$IzImage.hasClass("Right"))) {
						myApp.$IzMessageBox.izMessageBox("show", "这是第一页", {
							autoClose: true
						});
						return;
					}
			...
			$(".xxButton.page-Last").bind('click', function() {
				if (myApp.currentPageIndex === myApp.itemsDataSource.getRowCount()) {
					if (!myApp.$IzImage.hasClass("Splitted") || (myApp.$IzImage.hasClass(
							"Splitted") && myApp.$IzImage.hasClass("Left"))) {
						myApp.$IzMessageBox.izMessageBox("show", "这是最后一页", {
							autoClose: true
						});
						return;
					}
			...
			var rowCount = this.itemsDataSource.getRowCount(),
				pageText, elementsHtml = "";
			for (var i = 1; i <= rowCount; ++i) {
				...
				elementsHtml += "<option value='" + i + "'>第 " + pageText +
					" 页</option>";
			}
			...
			$(".xxRadio.custom-MaxWidth").bind('click', function() {
				fitCustomEventHelper(this, MyConfig.View_Custom_MaxWidth, {
					maxWidth: $(".txtCustom-MaxWidth").val()
				});
			});

			$(".xxRadio.custom-MaxHeight").bind('click', function() {
				fitCustomEventHelper(this, MyConfig.View_Custom_MaxHeight, {
					maxHeight: $(".txtCustom-MaxHeight").val()
				});
			});

			$(".xxRadio.custom-FixedSize").bind('click', function() {
				fitCustomEventHelper(this, MyConfig.View_Custom_FixedSize, {
					fixedSize: {
						w: $(".txtCustom-FixedSize-Width").val(),
						h: $(".txtCustom-FixedSize-Height").val()
					}
				});
			});

			$(".xxRadio.custom-Zoom").bind('click', function() {
				fitCustomEventHelper(this, MyConfig.View_Custom_Zoom, {
					zoom: $(".txtCustom-Zoom").val()
				});
			});
		...
		configure: function() {
			...
			if (IzConfig["Image-Split-Enable"]) {
				if (!myApp.$IzImage.hasClass("izImageEx") || !myApp.$IzImage.izImageEx(
						"isAutoSplit")) {
			...
			switch (IzConfig["Image-CustomMode"]) {
				case MyConfig.View_Custom_MaxWidth:
					$(".xxRadio.custom-MaxWidth").addClass("selected");
					break;
				case MyConfig.View_Custom_MaxHeight:
					$(".xxRadio.custom-MaxHeight").addClass("selected");
					break;
				case MyConfig.View_Custom_FixedSize:
					$(".xxRadio.custom-FixedSize").addClass("selected");
					break;
				case MyConfig.View_Custom_Zoom:
					$(".xxRadio.custom-Zoom").addClass("selected");
					break;
			}

			switch (IzConfig["Image-FitMode"]) {
				case MyConfig.View_IntrinsicSize:
					$(".xxRadio.fit-IntrinsicSize").addClass("selected");
					break;
				case MyConfig.View_BestFit:
					$(".xxRadio.fit-BestSize").addClass("selected");
					break;
				case MyConfig.View_Width:
					$(".xxRadio.fit-Width").addClass("selected");
					break;
				case MyConfig.View_Height:
					$(".xxRadio.fit-Height").addClass("selected");
					break;
				case MyConfig.View_Custom:
					$(".xxRadio.fit-Custom").addClass("selected");
					break;
			}
		...
		init: function() {
			...
			myApp.$IzMessageBox = $("<div id='mmMessageBox'></div>").izMessageBox({
				...
				buttons: [{
					...
					name: 'OK',
					click: function() {
						myApp.$IzMessageBox.izMessageBox("hide")
					}
				}]
			}).appendTo(document.body);
			...
			myApp.itemsDataSource
				.on("izSelectDataCompletedEvent.DataSource", function(data) {
					...
					for (var i = 0; i < data.rows.length; ++i) {
						$li = myApp.$PhotoGirdItems.eq(DataSource.repairIndexToZero(data.beginPageIndex) +
							i);
						$img = $li.find("img");
						if ($img.length) {
							$img.attr('src', data.rows[i].src);
						} else {
							$img = $("<img/>")
							$li.html(
								$img
				...
				.on("izSelectAllDataCompletedEvent.DataSource", function(data) {
					...
					for (var i = 0; i < data.rows.length; ++i) {
						htmlList += "<a title='mm' href='" + data.rows[i].src + "'>" + data.rows[
							i].src + "</a><br />";
					}
			...
			var $loadingImage = $(
				"<img alt='载入中...' title='载入中...' height=24 width=24 src='http://imageshack.us/a/img6/1610/loadingqv.gif' /> "
			).hide();
			var $loadedImage = $(
				"<img alt='载入完成' title='载入完成' height=24 width=24 src='http://imageshack.us/a/img834/75/donw.png' /> "
			).hide();
			var $brokenImage = $(
				"<img alt='载入错误' title='载入错误' height=24 width=24 src='http://imageshack.us/a/img853/2047/errorii.png' /> "
			).hide();

			myApp.$ControlPanelMessage.find(".mmImageStatus").append([$loadingImage,
				$loadedImage, $brokenImage
			]);

			myApp.$IzImage
				.bind("izBeforeLoadEvent.izImage", function(event, data) {
					if (data.newSrc != data.oldSrc) {
						$loadingImage.show();
						$loadedImage.hide();
						$brokenImage.hide();
					}
		...
		off: function(events, callback, context) {
			...
			while (event = events.shift()) {
				...
				for (i = list.length - 2; i >= 0; i -= 2) {
					if (!(callback && list[i] !== callback || context && list[i + 1] !==
							context)) {
						list.splice(i, 2);
					}
				}
	...
	MyUtils.extend(DataSource.prototype, BackboneEvents, {
		...
		selectData: function(begin, count) {
			var beginIndex = Math.max(DataSource.repairIndexToZero(begin), 0),
				...
				data = {
					beginPageIndex: begin,
					rows: this.dataTable.slice(beginIndex, endIndex)
				};
		...
		selectAllData: function() {
			var data = {
				rows: this.dataTable.slice(0, this.getRowCount())
			};
	...
	MyUtils.extend(DataSourceA.prototype, DataSource.prototype, {
		...
		linkCount: 1, // 每页可取得的圖片網址的數目 the number of image urls per page
		isUseMT: true,
		maxConnection: 3,
		activeConnections: {
			length: 0
			...
			for (var i = beginIndex; i < endIndex; ++i) {
				if (typeof this.dataTable[i] === 'undefined') {
					...
					var beginIndex2 = i,
						endIndex2 = i + 1,
						data = {
							beginPageIndex: DataSource.repairIndexToOne(i),
							rows: this.dataTable.slice(beginIndex2, endIndex2)
						};
		...
		_grabData: function(dataIndex) {
			var dataSource = this,
				pageIndex = DataSource.repairIndexToOne(dataIndex);
			...
			try {
				...
				console.log(e.name + ": " + e.message + " ( MangaYaki E3: " + (e.number &
					0xFFFF).toString() + " )");
				return;
			}
		},
		...
		selectAllDataST: function() {
			this.$IzMessageBox.izMessageBox("show", "图片列表获取中... ", {
				closeButton: false
			});
		...
		_grabDataST: function(dataIndex) {
			...
			try {
				...
				console.log(e.name + ": " + e.message + " ( MangaYaki E4: " + (e.number &
					0xFFFF).toString() + " )");
				return;
			}
		},
		...
		selectAllDataMT: function() {
			...
			*/
			...
			if (taskData.undefinedDataIndex.length) {
				this.$IzMessageBox.izMessageBox("show", "图片列表处理中... ", ["izAbort"]);

				var dataIndex = taskData.undefinedDataIndex.pop(),
					connectionCount = 0;
				...
				var data = {
					beginPageIndex: DataSource.repairIndexToOne(taskData.beginIndex),
					rows: this.dataTable.slice(taskData.beginIndex, taskData.endIndex)
				};
				this.trigger("izSelectAllDataCompletedEvent.DataSource", data);
			}
		},
		...
		_loadend: function(dataIndex, taskData) {
			...
			if (taskData.errorflag) {
				this.$IzMessageBox.izMessageBox("show", "图片列表取消中");
		...
		_grabDataMT: function(dataIndex, taskData) {
			...
			try {
				...
				console.log(e.name + ": " + e.message + " ( MangaYaki E2: " + (e.number &
					0xFFFF).toString() + " )");
				return;
			}
		}
	...
	//http://www.6comic.com/
	//http://www.8comic.com/
	if (/^http:\/\/\w+.(2|6|8)comic.com\//.test(gBaseUrl) || /comicvip.com/.test(
			gBaseUrl)) {
		var $oldImg = $("#TheImg");
		...
		if ($oldImg.length) {
			...
			DataSource.prototype.init = function() {
				...
				var si = function(c, p) {
					return 'http://img' + ss(c, 4, 2) + '.8comic.com/' + ss(c, 6, 1) + '/' +
						ti + '/' + ss(c, 0, 4) + '/' + nn(p) + '_' + ss(c, mm(p) + 10, 3, f) +
						'.jpg';
				};
			...
			var newApp = new MyApp({
				...
				historyUrl: function(p) {
					return "?ch=" + _unsafeWindow.ch + "-" + p;
				},
	...
	//http://99mh.com/
		...
		/^http:\/\/[\w]{0,3}[\.]?(99comic|99manga|cococomic|99mh|99770).(com|cc)/.test(
			gBaseUrl)) {
		if ($("#imgCurr").length) {
			var $oldImg = $("#imgCurr"),
				$newImg = $("<img id='mmImage' />").insertAfter($oldImg);
			$oldImg.remove();
			...
			DataSource.prototype.init = function() {
				...
				for (var i = 0; i < sFiles.length; ++i) {
					this.dataTable[i] = {
						src: server + sPath + sFiles[i]
					};
				}

				this.size = this.dataTable.length;

				if (/(dm.99manga)|(99mh)/.test(gBaseUrl)) {
					this.hasPrevVolume = true;
					this.hasNextVolume = true;

					var changeVol = function(type) {
						var sComicID = _unsafeWindow.ComicID || $("#hdInfoID").val();
						var sComicListID = _unsafeWindow.ComicListID || $("#hdID").val();

						var url = "/app/getNextVolUrl.aspx?ComicID=" + sComicID + "&VolID=" +
							sComicListID + "&t=" + type;
						var xhr = $.ajax({
							url: url,
							async: false
				...
				} else if (typeof GM_xmlhttpRequest !== 'undefined' && !window.chrome) {
					this.hasPrevVolume = true;
					this.hasNextVolume = true;

					var rewrite99URL = function(url) {

						var parsePathname = function(url) {
							var parser = document.createElement('a');
							parser.href = url;

							return parser.pathname.replace(/^\/|\/$/g, "").split("/");
						};

						var raw = parsePathname(url);
						var cid = raw[1],
							vid = raw[2];
						var p = parsePathname(gBaseUrl);

						if (/mh.99770/.test(gBaseUrl)) {
							p[1] = cid;
							p[2] = vid;
						} else if (/www.99comic/.test(gBaseUrl)) {
							p[1] = cid + "o" + vid;
						}

						return "http://" + location.host + "/" + p.join("/");
					};
					...
					var changeVol = function(type) {
						...
						if (/www.99comic/.test(gBaseUrl)) {
							sComicID = sComicID.toString().substring(2);
							sComicListID = sComicListID.toString().substring(sComicID.length + 1);
						}

						try {
							var xhr = GM_xmlhttpRequest({
								method: 'GET',
								url: "http://99mh.com/app/getNextVolUrl.aspx?ComicID=" + sComicID +
									"&VolID=" + sComicListID + "&t=" + type,
								headers: {
									'Referer': "http://99mh.com/comic/"
								},
								synchronous: true,
			...
			var newApp = new MyApp({
				...
				historyUrl: function(p) {
					return "?s=" + _unsafeWindow.getCSL() + "&p=" + p;
				},
				itemsDataSource: new DataSource
			});
			...
			DataSource.prototype.init = function() {
				var server = _unsafeWindow.ServerList[_unsafeWindow.server.replace("*v",
						"") - 1],
					arrPicListUrls;
				...
				for (var i = 0; i < arrPicListUrls.length; ++i) {
					this.dataTable[i] = {
						src: server + arrPicListUrls[i]
					};
				}

				this.size = this.dataTable.length;

				if (typeof GM_xmlhttpRequest !== 'undefined' && !window.chrome) {
					this.hasPrevVolume = true;
					this.hasNextVolume = true;
					...
					var rewrite99URL = function(url) {
						...
						if (/\/\/99comic/.test(gBaseUrl)) {
							p[1] = "99" + cid;
							p[2] = "list_" + vid + ".htm?s=" + _unsafeWindow.server;
						} else if (/\/\/(cococomic|99manga|99770)/.test(gBaseUrl)) {
							p[1] = cid;
							p[2] = vid + ".htm?s=" + _unsafeWindow.server;
						} else if (/www.cococomic/.test(gBaseUrl)) {
							p[1] = cid;
							p[2] = vid;
						}
						...
						try {
							var xhr = GM_xmlhttpRequest({
								...
								url: "http://99mh.com/app/getNextVolUrl.aspx?ComicID=" + sComicID +
									"&VolID=" + sComicListID + "&t=" + type,
								headers: {
									'Referer': "http://99mh.com/comic/" + sComicID + "/" +
										sComicListID
								},
								synchronous: true,
								onload: function(resp) {}
			...
			var newApp = new MyApp({
				...
				historyUrl: function(p) {
					return "?v=" + p + "*s=" + _unsafeWindow.server;
				},
				itemsDataSource: new DataSource
			});
	...
	//http://tel.dm5.com/
	//http://cnc.dm5.com/
	else if (/^http:\/\/[\w]+.dm5.com\//.test(gBaseUrl)) {
		if ($("#pagelist").length) {
			var $oldImg = $("#showimage"),
				$newImg = $("<img id='mmImage' />").insertBefore($oldImg);
			$oldImg.remove();
			...
			DataSourceA.prototype.init = function() {
				...
				this._getUrl = function(page) {
					return "imagefun.ashx?d=" + (+new Date) + "&cid=" + _unsafeWindow.DM5_CID +
						"&page=" + page + "&key=" + this.mkey + "&maxcount=10&language=1";
				};
				...
				this.getNextVolume = function() {
					var results =
						/\u4e0b\u4e00\u7ae0\u8282\uff1a<span><a class="redzia" href="(\/m[\d]+\/)">/
			...
			var newApp = new MyApp({
				...
				historyUrl: function(p) {
					return _unsafeWindow.DM5_COMIC_URL + "#ipg" + p;
				},
	...
	// else if (/^http:\/\/[\w]+.dmzj.com\//.test(gBaseUrl)) {
	//
	// 			if ($(".select_jump").childNodes.length) {
	// 				var $newImg = $("#pic");
	//
	// 				DataSourceA.prototype.init = function() {
	// 					this.name = "dmzj";
	// 					this.dataTable = new Array($(".select_jump").childNodes.length);
	// 					this.size = $(".select_jump").childNodes.length;
	//
	// 					this.hasNextVolume = true;
	//
	// 					this.linkCount = 2;
	...
	//
	// 					this._getUrl = function(page) {
	// 						return "imagefun.ashx?cid=" + _unsafeWindow.cid + "&page=" + page +
	// 							"&key=" + this.mkey + "&maxcount=10";
	// 					};
	//
	// 					this._parse = function(responseText, page) {
	// 						eval(responseText);
	...
	//
	// 					this.getNextVolume = function() {
	// 						var $a = $("#divEnd .tk22 a:first-child");
	//
	...
	//http://tel.1kkk.com/
	//http://cnc.1kkk.com/
	else if (/^http:\/\/[\w]+.1kkk.com\//.test(gBaseUrl)) {
		if ($("#pagelist").length) {
			var $newImg = $("<img id='mmImage' />");

			$("<div id='mmBox' />").css({
				"text-align": "center"
			}).append($newImg).insertAfter($("#chapterpager"));
